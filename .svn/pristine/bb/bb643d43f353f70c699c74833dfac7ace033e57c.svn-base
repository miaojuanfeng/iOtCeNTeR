...
public class LoginController extends BaseController {
    ...
    public ReturnBean login(LoginFormDTO loginFormDTO) {
        ...
        try {
            //开启验证码
            if (ifVerifyCode) {
                String ticket = (String) ShiroUtils.getSessionAttribute(GlobalConstant.KAPTCHA_SESSION_KEY);
                log.debug("验证码:{}", ticket);
                if (!loginFormDTO.getTicket().equalsIgnoreCase(ticket)) {
                    return ReturnBean.error("验证码错误");
                }else{
                    Objects.requireNonNull(ShiroUtils.getSession()).removeAttribute(GlobalConstant.KAPTCHA_SESSION_KEY);
                }
            }
            // shiro的登录
